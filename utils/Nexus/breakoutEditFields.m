function entryParams = breakoutEditFields(nexon, entryPanel, cfgParams, entryChangedFcn, hasParent)
    ...
    for i=1:length(entryFields)
        ...
        switch class(value)
            ...
            case "string"
                if size(value,1) == 1
                    entryPanel.Panel.(editField).uiField = uieditfield(entryPanel.Panel.ph,"text", "Position", [4,panelH-(m+1)*25,panelW*0.8,panelH/30], "Value", array2string(cfgParams.(editField)),"BackgroundColor",[0,0,0],"FontColor",nexon.settings.Colors.cyberGreen,"ValueChangedFcn",@(~,~)entryChangedFcn(nexon, entryPanel, editField));
                    entryParams.(editField) = value;
                elseif size(value,1) > 1
                    entryPanel.Panel.(editField).uiField = uidropdown(entryPanel.Panel.ph,"Position",[4,panelH-(m+1)*25,panelW*0.8,panelH/30],"Value",value(1),"Items",value,"BackgroundColor",nexon.settings.Colors.cyberGreen,"FontColor",[0,0,0],"ValueChangedFcn",@(~,~)entryChangedFcn(nexon, entryPanel, editField));
