function nexFigure_stateSpace(nexObj)
    ...
    % canvas: full point cloud (D1-gradient dimmed).
    % canvas_trail: after-image overlay at ptr.value; always rendered, VW-independent.
    % canvas_tracker: per-VW-group overlays managed by rebuildTrackers().
    nexObj.Figure.panel0.tiles.graphics.canvas = scatter3(ax, [], [], [], ...
        100, C.cyberGreen, "filled");
    hold(ax, 'on');
    nexObj.Figure.panel0.tiles.graphics.canvas_trail = scatter3(ax, [], [], [], ...
        150, C.cyberGreen, "filled");
    hold(ax, 'off');
    ...
    nexObj.Figure.reloadButton = uibutton(nexObj.Figure.fh, ...
        "Position",        [275, 650, 70, 25], ...
        "BackgroundColor", [0,0,0], ...
        "FontColor",       C.cyberGreen, ...
        "Text",            "Reload DF", ...
        "Tooltip",         "Re-read DF from router and rebuild STATE", ...
