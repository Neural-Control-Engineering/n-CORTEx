function nexFigure_categorical(nexObj)
    ...
    %% pooling panel    
    panel2.ph = uipanel(nexObj.Figure.panel1.ph,"Position",[5,1120,wInner,275],"BackgroundColor",[0,0,0]);
    ...
    panel3.ph = uipanel(nexObj.Figure.panel1.ph,"Position",[5, 845, wInner, 275],"BackgroundColor",[0,0,0],"Scrollable","on");
    nexObj.Figure.panel3 = nexObj_listCfgPanel(nexObj.nexon, panel3, nexObj.selectionBus.categories,[]);
    % items selection
    panel4.ph = uipanel(nexObj.Figure.panel1.ph,"Position",[5, 565, wInner, 275], "BackgroundColor",[0,0,0],"Scrollable","on");    
    nexObj.Figure.panel4 = nexObj_listCfgPanel(nexObj.nexon, panel4, nexObj.selectionBus.items,[]);
    % compFcn panel (if entered)
    panel5.ph = uipanel(nexObj.Figure.panel1.ph,"Position",[5, 285, wInner, 275], "BackgroundColor",[0,0,0],"Scrollable","on");    
    % opFcn panel (if entered)    
    panel6.ph = uipanel(nexObj.Figure.panel1.ph,"Position",[5, 5, wInner, 275], "BackgroundColor",[0,0,0]);    
    %% UI CONTROL
    ...
    % dfID control
    nexObj.Figure.dfIDEditField = uieditfield(nexObj.Figure.fh,"BackgroundColor",[0,0,0],"FontColor",nexObj.nexon.settings.Colors.cyberGreen,"Position",[710, 690, wInner, 25], "Value",nexObj.dfID_source,"ValueChangedFcn",@(src,event)updateDfIDFcn(src,event,nexObj.nexon,nexObj));
