classdef nexObj_poolMap_chans < nexObj_poolMap
    methods
        ...
        %             regions = convertCharsToStrings(pMap.Map.mapID)';
        %             % edges = ([size(regions,2) - ([1, find(~strcmp(regions(1:end-1), regions(2:end))) + 1]-1),0])';
        %             edges = pMap.axID; % store boundary locations
        %             % impute by divisions per bin
        %             edgeCmp = num2cell([edges(1:end-1), edges(2:end)],2);
        %             edgeSubDivs = cellfun(@(row) flip(round(linspace(row(2), row(1),pMap.divsPerBin+1)',"TieBreaker","fromzero"),1), edgeCmp, "UniformOutput", false);
