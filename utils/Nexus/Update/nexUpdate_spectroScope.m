function nexUpdate_spectroScope(nexObj)
    ...
    try % RECOVERY
        try
            nexObj.DF = dtsIO_readDF(nexObj.nexon, nexObj.dfID_target,[]);
        catch
            nexObj.DF = retrieveFrameBuffer(nexObj);
        end
    catch % COMPUTE NEW
    ...
    DF_pooled = DF_form;
    [DF_pooled.df, binIDs_chans, binTicks_chans] = structfun(@(df) nexAnalysis_averagePool(df, nexObj.pMap_chans,1, DF.ax.chans), DF_pooled.df,"UniformOutput",false);
    [DF_pooled.df, binIDs_freqs, binTicks_freqs] = structfun(@(df) nexAnalysis_averagePool(df, nexObj.pMap_freqs,2, DF.ax.f), DF_pooled.df,"UniformOutput",false);
    % need to omit nans
