...
def benchmarkLatency(model, num_channels_list, num_iterations=100):
    ...
    plt.xlabel('Number of Channels', color='white', fontsize=16)
    plt.ylabel('Latency (seconds)', color='white', fontsize=16)
    plt.title('Model Latency vs Number of Channels', color='white', fontsize=20)
    plt.legend(facecolor='#98b69e', edgecolor='white', labelcolor='white', fontsize=14)
    plt.grid(False)  # Make gridlines invisible
    plt.gca().spines['bottom'].set_color('white')
    plt.gca().spines['top'].set_color('white') 
    plt.gca().spines['right'].set_color('white')
    plt.gca().spines['left'].set_color('white')
    plt.gca().tick_params(axis='x', colors='white', labelsize=14)
