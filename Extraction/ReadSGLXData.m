...
% much easier!
%
function [sglx_data] = ReadSGLXData(binName, path, ch_cnt)

% Ask user for binary file
%[binName,path] = uigetfile('*.bin', 'Select Binary File');

% Parse the corresponding metafile
sglx_data.meta = ReadMeta(binName, path);

% Get first one second of data
nSamp = floor(str2num(sglx_data.meta.fileTimeSecs) * SampRate(sglx_data.meta));
sglx_data.dataArray = ReadBin(0, nSamp, sglx_data.meta, binName, path);
...
if dataType == 'A'
    if strcmp(sglx_data.meta.typeThis, 'imec')
        sglx_data.dataArray = GainCorrectIM(sglx_data.dataArray, [ch], sglx_data.meta);
    else
        sglx_data.dataArray = GainCorrectNI(sglx_data.dataArray, [ch], sglx_data.meta);
    end
    %plot(sglx_data.dataArray(ch,:));
else
    sglx_data.digArray = ExtractDigital(sglx_data.dataArray, sglx_data.meta, dw, dLineList);
    for i = 1:numel(dLineList)
        %plot(sglx_data.digArray(i,:));
        hold on
    end
    hold off
end
